...
def image_aspect_change(image_loc):
    ...
    my_pic = my_pic.resize((width,height),Image.ANTIALIAS)
    my_pic.save(image_loc)


def final_image_step(photo,caption):
    base = Image.open(photo).convert('RGB').save("image_name.jpg")
...
def image_handle(photo_url, photo_loc, caption = None):
    ...
    base = Image.open(photo_loc).convert('RGB').save("image_name.jpg")
